...
import { PlayCircle, Menu, Home, HelpCircle, Shield, BookOpen, FileText } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        {/* Platform switcher pill — desktop */}
        <div className="hidden md:flex items-center gap-1 rounded-full border border-border bg-muted p-1 text-sm">
          <Link href="/" className="rounded-full px-4 py-1.5 font-medium text-muted-foreground transition-colors hover:bg-background hover:text-foreground">
            🎵 TikTok
          </Link>
        ...
        {/* Mobile: language switcher OUTSIDE hamburger + hamburger */}
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[350px]">
              <div className="flex flex-col gap-6 pt-6">
                <Link href="/douyin" className="flex items-center gap-2 text-xl font-bold" onClick={() => setIsOpen(false)}>
                  <PlayCircle className="h-7 w-7 text-red-500" />
                  <span>Snap<span className="text-red-500">Yin</span></span>
                </Link>
                ...
                <div className="flex items-center gap-1 rounded-full border border-border bg-muted p-1 text-sm w-fit">
                  <Link href="/" className="rounded-full px-4 py-1.5 font-medium text-muted-foreground" onClick={() => setIsOpen(false)}>
                    🎵 TikTok
                  </Link>
                  <span className="rounded-full px-4 py-1.5 font-medium bg-background text-foreground shadow-sm">
                    🎬 Douyin
